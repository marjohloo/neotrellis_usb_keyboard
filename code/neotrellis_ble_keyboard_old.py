...
# Boot states
BOOT_STATE_USB = 0
BOOT_STATE_BLE_DIS = 1
BOOT_STATE_BLE_CON = 2

# this will be called when button events are received during boot
def key_event_boot(event):
    if BLE_LIVE:
        if ble.connected:
            if event.edge == NeoTrellis.EDGE_RISING:
                for connection in ble.connections:
                    connection.disconnect()
                boot_state = BOOT_STATE_BLE_DIS
                # Convert the hue to RGB values.
                r, g, b = hsv_to_rgb(boot_hue[boot_state], 1.0, VAL_OFF)
                for i in range(16):
                    # Finally set the LED
                ...
                if not ble.connected:
                    if not ble.advertising:
                        print("BLE disconnect advertising")
...
# BLE setup
    ...
    if ble.connected:
        ...
        print("BLE intialise advertising x")
        ble.start_advertising(advertisement, scan_response)
        boot_state = BOOT_STATE_BLE_DIS
        # Convert the hue to RGB values.
        r, g, b = hsv_to_rgb(boot_hue[boot_state], 1.0, VAL_OFF)
...
for i in range(16):
    if BLE_LIVE:
        if ble.connected:
            ...
            trellis.callbacks[i] = key_event_boot

# Boot loop USB or BLE connected 10s
if boot_delay != None:
    time_now = time.monotonic()
    time_exit = time_now + boot_delay
    i = 15
    while time_now < time_exit:
        ...
        if i > 15:
            i = 0
        # Convert the hue to RGB values.
        r, g, b = hsv_to_rgb(boot_hue[boot_state], 1.0, VAL_ON)
        # Finally set the LED
        trellis.pixels[i] = (r, g, b)
